...
public class Vector3 {
    ...
    public Vector3 rotate(float angle, float axcisX, float axcisY, float axcisZ){
        inVec[0] = x;
        inVec[1] = y;
        inVec[2] = z;
        inVec[3] = 1;
        ...
        Matrix.rotateM(matrix, 0, angle, axcisX, axcisY, axcisZ);
    ...
    public float angle(float firstAxcis, float secondAcscis ){
        float angle = (float)Math.atan2(secondAcscis, firstAxcis) * TO_DEGREES;
        if(angle < 0){
            angle += 360;
        }
        return angle;
